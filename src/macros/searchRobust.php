 ...
 */
Collection::macro('searchRobust', function ($haystack, $needle, $ith = null, $nth = null, $depth = null, $level = 0, $notation = '') {
    $level += 1;
    $count = 1;
    $length = 0;
    $output = [];
        ...
        if (is_array($haystack) && count($haystack) > 0) {
            foreach ($haystack as $key => $value) {
                if ($value == $needle) {
                    ...
                    list($result, $atDepth) = $this->searchRobust($value, $needle, $ith, $nth, $depth, $level, $key);
